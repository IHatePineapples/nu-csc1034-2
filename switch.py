...
class Switch:
    ...
    def run_game(self):
        ...
        while True:
            ...
            if choice == 1:
                ...
                self.players = []
                # load each players' (object) attributes and put them in the list self.players
                for i in range(0, len(player_info)):
                    if player_info[i][0] == 'human':
                        human_player = player_classes['human'](player_info[i][1])
                        self.players.append(human_player)
                    elif player_info[i][0] == 'simple':
                        simple_player = player_classes['simple'](player_info[i][1])
                        self.players.append(simple_player)
                    elif player_info[i][0] == 'smart':
                        smart_player = player_classes['smart'](player_info[i][1])
    ...
    def run_player(self, player):
        ...
        if self.skip:
            ...
            self.skip = False
            UI.print_message(f"{player.name} is skipped.")
            return False
        if self.draw2:
            # draw two cards
            picked = self.pick_up_card(player, 2)
            self.draw2 = False
            UI.print_message(f"{player.name} draws {picked} cards.")
        if self.draw4:
            # draw four cards
